...
**Agentes:** para validar rotas com banco real, use `docker compose up -d db` e defina `TEST_DATABASE_URL`. Para testar o PWA no browser com roteamento de produção, use `docker compose -f docker-compose.dev.yml up --build` (ver seção abaixo). Não commite `.env` com credenciais.
...
| Fluxo                          | Quando                                                                             | Subir                                                 |
| ------------------------------ | ---------------------------------------------------------------------------------- | ----------------------------------------------------- |
| **Caddy (`clandestino.test`)** | Testar UI no browser com proxy igual à produção; não subir `api`/`web` manualmente | `docker compose -f docker-compose.dev.yml up --build` |
...
# parar (volume ./clandestino-db preservado)
...
| Seed              | `SEED_ON_START=true` (opcional; padrão `false`)               |
...
| Docker / Compose                | `docker-compose.yml`, `docker-compose.dev.yml`, `README.md` |
